...
CAS-numbers, Pubchem IDs (CID), HMDB, KEGG, and ChEBI identifiers is accomplished
through a comprehensive mapping table with nearly 900.000 compounds that is part
of the package.
...
First, the `multiGSEA` package is part of [Bioconductor](https://www.bioconductor.org/). 
Hence, the best way to install the package is via `BiocManager`. Start R (>=4.0.0) and run the following commands:
...
A summary of supported organisms, they naming format within `multiGSEA` and
...
caption <- "Table summarizing the top 15 pathways. Pathways from KEGG, Reactome,
...
knitr::kable( 
    ...
    dplyr::select( c( transcriptome_padj, proteome_padj, metabolome_padj, combined_padj)) %>% 
    dplyr::slice( 1:15),
    caption = caption
)
```
